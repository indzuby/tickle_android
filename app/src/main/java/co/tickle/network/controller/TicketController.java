...
public class TicketController extends BaseController {
    ...
    public void getMyList(String category,boolean favorite, Callback<TicketListResponseForm> callback) {
        String token = SessionUtils.getString(context, CodeDefinition.TOKEN, RestApi.TESTER1);
        Call<TicketListResponseForm> call = ticketService.getMyList(token, category,favorite);
        call.enqueue(callback);
    }
    ...
    public void favorite(String ticketId, boolean favorite, Callback<ResponseForm> callback) {
        String token = SessionUtils.getString(context, CodeDefinition.TOKEN, RestApi.TESTER1);
        Call<ResponseForm> call = ticketService.favorite(token, ticketId, favorite);
        call.enqueue(callback);
    }
